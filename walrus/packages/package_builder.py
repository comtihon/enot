...
def walrusify(path):   # TODO no need to write files in tmp projects
...
def build_package(path: str, walrus_global_config: WalrusGlobalProperties, package_config=None) -> bool:
    ...
    for dep in package_config.deps:
        ...
        if not package_cache.exists(package):
            build_dep(package_cache, package, walrus_global_config)
...
def build_dep(package_cache: Cache, package: WalrusPackage, walrus_global_config: WalrusGlobalProperties):
